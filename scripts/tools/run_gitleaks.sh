...
GITLEAKS_CONFIG_PATH="${GITLEAKS_CONFIG_PATH:-/SimpleSecCheck/config/tools/gitleaks/config.yaml}"
...
if command -v gitleaks &>/dev/null; then
  ...
  gitleaks detect --source "$TARGET_PATH" --report-path "$GITLEAKS_JSON" --config "$GITLEAKS_CONFIG_PATH" --no-git 2>/dev/null || {
  ...
  gitleaks detect --source "$TARGET_PATH" --config "$GITLEAKS_CONFIG_PATH" --no-git --verbose > "$GITLEAKS_TEXT" 2>/dev/null || {
