...
async def start_batch_scan(
    ...
    async with _batch_scan_lock:
        ...
        # Validate scan type
        if request.scan_type not in ["code", "image", "website", "network"]:
        ...
        for repo_url in request.repositories:
            ...
            if request.scan_type == "code" and not is_git_url(repo_url):
                ...
                continue
            if request.scan_type == "image" and not repo_url:
                print(f"[Batch Scan] Skipping invalid image reference: {repo_url}")
                continue
