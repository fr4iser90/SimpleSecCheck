...
class ClairScanner(BaseScanner):
    ...
    def __init__(
        ...
        """
        super().__init__("Clair", target_path, results_dir, log_file, config_path)
        self.clair_image = clair_image or os.getenv("CLAIR_IMAGE", "")
    ...
    def scan(self) -> bool:
        ...
        if not self.clair_image:
            self.log("No container image specified, skipping Clair scan.", "WARNING")
            return True
...
if __name__ == "__main__":
    ...
    config_path = os.getenv("CLAIR_CONFIG_PATH", "/SimpleSecCheck/config/tools/clair/config.yaml")
    clair_image = os.getenv("CLAIR_IMAGE", "")
