...
def android_manifest_summary(json_path):
    """
    Extract findings from Android manifest JSON.
    ...
    try:
        ...
        for manifest_file in data.get("findings", []):
            for issue in manifest_file.get("security_issues", []):
                finding = {
                    "severity": issue.get("severity", "INFO"),
                    "type": issue.get("type", "unknown"),
                    "description": issue.get("description", ""),
                    "file": manifest_file.get("file", "unknown")
                }
...
REPORT_PROCESSOR = ReportProcessor(
    ...
    html_func=generate_android_manifest_html_section,
