...
class AnchoreScanner(BaseScanner):
    ...
    def __init__(
        ...
        """
        default_config = "/SimpleSecCheck/scanner/scanners/anchore/config/config.yaml"
    ...
    def scan(self) -> bool:
        ...
        if self.target_type and self.target_type != TargetType.DOCKER_IMAGE.value:
            self.log("TARGET_TYPE is not docker_image, skipping Anchore scan.", "INFO")
            return True
...
if __name__ == "__main__":
    ...
    config_path = os.getenv("ANCHORE_CONFIG_PATH", "/SimpleSecCheck/scanner/scanners/anchore/config/config.yaml")
