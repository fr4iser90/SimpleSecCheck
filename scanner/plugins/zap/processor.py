...
def generate_zap_html_section(zap_data, zap_html_path, Path_module, os_module):
    ...
    elif Path_module(os_module.path.join(os_module.environ.get('RESULTS_DIR', '/app/results'), "zap-report.html")).exists():
        html_parts.append(f'<p>See full ZAP report: <a href="zap-report.html">zap-report.html</a></p>')
    return "".join(html_parts)

REPORT_PROCESSOR = ReportProcessor(
    name="ZAP",
    summary_func=zap_summary,
    html_func=generate_zap_html_section,
    ai_normalizer=lambda data: [
        {
            "tool": "ZAP",
            "severity": str(alert.get("riskdesc", "")).upper(),
