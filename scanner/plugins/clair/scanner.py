...
class ClairScanner(BaseScanner):
    ...
    def __init__(
        ...
        """
        default_config = "/app/scanner/plugins/clair/config/config.yaml"
    ...
    def scan(self) -> bool:
        ...
        if self.target_type and self.target_type != TargetType.CONTAINER_REGISTRY.value:
            self.log("TARGET_TYPE is not container_registry, skipping Clair scan.", "INFO")
            return True
...
if __name__ == "__main__":
    ...
    config_path = os.getenv("CLAIR_CONFIG_PATH", "/app/scanner/plugins/clair/config/config.yaml")
