...
class DatabaseAdapter:
    ...
    async def create_tables(self):
        """Run Alembic upgrade head. Schema is versioned only via Alembic (no create_all)."""
        from infrastructure.database.alembic_runner import run_alembic_upgrade
        await run_alembic_upgrade()

    async def _migrate_existing_tables(self):
        """No schema changes here – use Alembic. Reserved for optional data-only migrations only."""
