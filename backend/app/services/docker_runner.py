...
try:
    ...
    get_host_project_root = None
    get_docker_compose_file = None
...
class DockerRunner:
    ...
    def determine_scan_type(self, target: str) -> Tuple[str, str, str, str]:
        """
        ...
        """
        target_info = classify_target_from_target(target)
        if target_info.is_network:
            return ("network", "", "", "network-infrastructure")
        if target_info.is_website:
        ...
        if target_info.is_image:
            return ("image", "", target, target.replace("/", "_"))
    ...
    def build_docker_compose_command(
        ...
        """
        if not get_docker_compose_file:
            raise RuntimeError("Scanner path helpers unavailable in this container")
        ...
        if scan_type == "image" and zap_target:
            env_vars.append(("SCAN_TARGET", zap_target))
        ...
        # Scanner asset volumes from manifests
        if ScannerAssetsManager and get_host_project_root:
            host_project_root = get_host_project_root() or os.environ.get("HOST_PROJECT_ROOT")
            if host_project_root:
                manager = ScannerAssetsManager(Path(host_project_root) / "scanner" / "scanners")
                for manifest in manager.load_manifests().values():
                    for asset in manifest.assets:
                        host_path = manager.resolve_host_path(Path(host_project_root), asset)
                        volumes.append((str(host_path), asset.mount.container_path))
            else:
                self.log_message("[Assets Volume] HOST_PROJECT_ROOT not set - skipping asset mounts", "WARNING")
    ...
    async def run_scan_async(
        ...
        # Determine results directory (always include project_dir to avoid polluting root)
        if not results_dir:
            # Use environment variable or calculate
            results_dir_env = os.environ.get("RESULTS_DIR")
            if results_dir_env:
                results_dir = os.path.join(results_dir_env, project_dir)
                self.log_message(
                    f"Using RESULTS_DIR from environment with project subdir: '{results_dir}'"
                )
            else:
