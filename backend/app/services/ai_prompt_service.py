...
try:
    scanner_root = os.getenv("SCANNER_ROOT")
    if scanner_root:
...
try:
    ...
    def load_policy(_: str) -> Dict:
        return {}

    def apply_semgrep_policy(findings: List[Dict], _: Dict) -> tuple[List[Dict], List[Dict]]:
        return findings, []

    def apply_gitleaks_policy(findings: List[Dict], _: Dict) -> tuple[List[Dict], List[Dict]]:
        return findings, []

    def apply_bandit_policy(findings: List[Dict], _: Dict) -> tuple[List[Dict], List[Dict]]:
        return findings, []
