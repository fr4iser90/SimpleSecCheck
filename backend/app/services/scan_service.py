...
try:
    ...
    get_logs_dir_for_scan = None
    get_scan_results_dir_host = None
    get_target_mount_path_host = None
    get_config_path_host = None
    get_finding_policy_check_path_for_git_clone = None
    get_finding_policy_check_path_for_local_scan = None
    ScannerAssetsManager = None


class ScanRequest(BaseModel):
    type: str  # code, image, website, network
    target: str
...
async def start_scan(
    ...
    # Validate scan type
    if request.type not in ["code", "image", "website", "network"]:
    ...
    target_info = classify_target(request.type, clean_target)
    if request.type == "code" and target_info.is_git:
    ...
    # Re-evaluate target classification after potential git clone
    target_info = classify_target(request.type, clean_target)

    # STEP 5: Validate target
    is_image_target = target_info.is_image
    if request.type in ["code", "image"] and not clean_target:
    ...
    # Scanner asset volumes
    assets_volume = []
    host_project_root = os.getenv("HOST_PROJECT_ROOT")
    if ScannerAssetsManager and host_project_root:
    ...
    # Volume mounts
    if request.type == "code" and not is_image_target:
